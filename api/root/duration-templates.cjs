...
router.get('/', async (req, res) => {
  try {
    const templates = await req.prisma.durationTemplate.findMany({
      orderBy: { id: 'asc' }
    });
    res.json(templates);
  } catch (error) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const template = await req.prisma.durationTemplate.create({
      data: {
        name,
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const template = await req.prisma.durationTemplate.update({
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await req.prisma.durationTemplate.delete({
      where: { id: parseInt(id) }
    });
