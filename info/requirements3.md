...
| 5 | Полный CRUD API для шаблонов длительности голосования | Реализовать создание, чтение, обновление и удаление шаблонов длительности через UI с хранением в базе данных | ✅ | 47f8de5, 1074f09, a0b75d5 | **Проблема**: API /api/duration-templates возвращал ошибку "Cannot read properties of undefined (reading 'findMany')". Отображались "NaN мин" и "0 сек" вместо реальных значений. Отсутствовали методы POST/PUT/DELETE. **Причина**: Модель DurationTemplate отсутствовала в Prisma schema, поэтому req.prisma.durationTemplate был undefined, хотя таблица существовала в БД. **Решение (Коммит 47f8de5)**: Frontend (src/pages/DurationTemplatesPage.jsx) - исправлено отображение длительности с добавлением нормализации Number(t.durationInSeconds) \|\| 0 и проверки на NaN в formatDuration(). **Решение (Коммит 1074f09)**: Backend (api/root/duration-templates.cjs) полностью переписан для работы с Prisma: GET / - получение всех шаблонов из DurationTemplate, POST / - создание нового шаблона, PUT /:id - обновление шаблона, DELETE /:id - удаление шаблона. В api/server.cjs:369 добавлена передача prisma в роутер. **Решение (Коммит a0b75d5)**: Добавлена модель DurationTemplate в prisma/schema.prisma (lines 217-223) с полями id, name, durationInSeconds, createdAt, updatedAt. Выполнен npx prisma generate для обновления Prisma Client. **Затронутые файлы**: - prisma/schema.prisma:217-223 (новая модель DurationTemplate) - api/root/duration-templates.cjs (полный CRUD API) - api/server.cjs:369 (передача prisma в роутер) - src/pages/DurationTemplatesPage.jsx:36,108-109 (исправление отображения) **Результат**: ✅ req.prisma.durationTemplate.findMany() работает без ошибок ✅ Шаблоны загружаются из БД (DurationTemplate table) ✅ Можно создавать новые шаблоны через UI ✅ Можно редактировать существующие шаблоны ✅ Можно удалять шаблоны ✅ Отображается корректная длительность: "30 сек", "1 мин", "10 мин" и т.д. |
|   |           |          |        |        |               |
