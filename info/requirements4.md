# Тестирование логики процедур голосования
...
**Логика:**
- Условие 1: `(Все пользователи заседания) * 66.67% > За` → `За > 66.67% от всех`
- Условие 2: `За > Против`
- Решение: Принято, если ОБА условия выполнены (И)

**Математика:**
- Для принятия нужно: `За > (Всего депутатов * 0.6667)` И `За > Против`
- Пример: Всего 38 депутатов → нужно минимум 26 голосов ЗА (т.к. 25.3346 округляется вверх)

### Процедура #4: Большинство от установленного числа депутатов

**Условия процедуры:**
```json
[
  {"elements": ["Все пользователи заседания", "*", 50, ">", "За"], "operator": null},
  {"elements": ["За", ">", "Против"], "operator": null}
]
```

**Логика:**
- Условие 1: `(Все пользователи заседания) * 50% > За` → `За > 50% от всех`
- Условие 2: `За > Против`
- Решение: Принято, если ОБА условия выполнены (И)

**Математика:**
- Для принятия нужно: `За > (Всего депутатов * 0.5)` И `За > Против`
- Пример: Всего 38 депутатов → нужно минимум 20 голосов ЗА (т.к. 19.01 > 19.0)

---

## Таблица тестовых сценариев

| # | Процедура | Всего депутатов | Присутствует (онлайн) | За | Против | Воздержались | Ожидаемый результат | Фактический результат | Статус | Комментарий |
|---|-----------|-----------------|----------------------|----|----|--------------|---------------------|----------------------|--------|-------------|
| **Процедура #2: Большинство от присутствующих** |||||||||
| 1 | Большинство от присутствующих | 38 | 10 | 5 | 5 | 0 | Не принято | | ⏳ | 5 не больше 50% от 10 (нужно 6+) |
| 2 | Большинство от присутствующих | 38 | 10 | 3 | 7 | 0 | Не принято | | ⏳ | 3 меньше 50% от 10 И 3 < 7 |
| 3 | Большинство от присутствующих | 38 | 10 | 6 | 4 | 0 | Принято | | ⏳ | 6 больше 50% от 10 (5) И 6 > 4 |
| **Процедура #3: Не менее 2/3 от установленного числа** |||||||||
| 4 | Не менее 2/3 от установленного | 38 | - | 25 | 13 | 0 | Не принято | | ⏳ | 25 не больше 66.67% от 38 (25.3346), нужно 26+ |
| 5 | Не менее 2/3 от установленного | 38 | - | 26 | 12 | 0 | Принято | | ⏳ | 26 больше 66.67% от 38 (25.3346) И 26 > 12 |
| 6 | Не менее 2/3 от установленного | 38 | - | 19 | 19 | 0 | Не принято | | ⏳ | 19 меньше 66.67% от 38 (25.3346) |
| **Процедура #4: Большинство от установленного числа** |||||||||
| 7 | Большинство от установленного | 38 | - | 19 | 19 | 0 | Не принято | | ⏳ | 19 не больше 50% от 38 (19), нужно 20+ |
| 8 | Большинство от установленного | 38 | - | 20 | 18 | 0 | Принято | | ⏳ | 20 больше 50% от 38 (19) И 20 > 18 |
| 9 | Большинство от установленного | 38 | - | 30 | 8 | 0 | Принято | | ⏳ | 30 больше 50% от 38 (19) И 30 > 8 |
| 10 | Большинство от установленного | 38 | - | 18 | 20 | 0 | Не принято | Принято | ❌ | 18 меньше 50% от 38 (19) И 18 < 20 (условие 2), но система вернула "Принято" |
...
**Корень проблемы:** Оператор между блоками берется из `conditions[blockIndex - 1].operator`, НО у процедур #2, #3, #4 ОБА блока имеют `operator: null`!
...
Choices: `FOR` (За), `AGAINST` (Против), `ABSTAIN` (Воздержался)
